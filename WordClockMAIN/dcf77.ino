int TimeoutDCF = 0; //Init timer for dynamic searching
int TimeTillTimeout = TimeoutDCF77Setting;

void DCF77Init() {
  if (DCFMode = 2) {
  delay(50);
  TimeTillTimeout + 17000000;
  delay(50);
  }
  else {
  delay(50);
...
void getSignal() {
  ...
  Serial.println(TimeTillTimeout);
  while (signalFound == false) 
  {
    old_loop();
    if ((year != 0) && (TimeoutDCF <= TimeTillTimeout)) {
      Serial.println(year);
      signalFound = true;
    }
  }
...
void old_loop() {
  //Serial.println("DCF77 encoding signal");
  //Serial.println(ss);
  if (ss != previousSecond) {
    serialDumpTime();
    previousSecond = ss;
  }
  Serial.println(DCFSignalState);
  Serial.println(previousSignalState);
  Serial.println(TimeoutDCF);  
  TimeoutDCF++;
